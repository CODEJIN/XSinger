...
# GTSinger's Japanese phoneme is not correct.
# Perhaps, kanji is converted to phoneme character by character, not sentence.
# (ex. '光る' is not 'çi̥kaɾʲiɾɯ', 'çi̥kaɾɯ')

def remove_diacritics(ipa: str):
    return ipa.replace('_ja', '').replace('\u0325', '').replace('ː', '')

def _Syllablize(
    pronunciation: list[str]
    ) -> List[Tuple[List[str], str, List[str]]]:
    use_vowels = ['a', 'e', 'i', 'o', 'ɨ', 'ɯ']
    ...
    while index < len(pronunciation):
        ...
        if phoneme == 'ɴ':
            if len(onsets) > 0:
                assert False            
            syllable_list.append(([], '', [phoneme]))
        elif not phoneme in use_vowels:
    ...
    if len(syllable_list) == 0:
        syllable_list.append((onsets, '', []))        
    elif 'ɴ' in syllable_list[-1][2] and len(onsets) > 0:
        print(syllable_list)
        print(onsets)
        assert False
    elif not 'ɴ' in syllable_list[-1][2]:
